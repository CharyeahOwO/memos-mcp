...
Current state: **Step 1 (base local retrieval module) is implemented and verified against a real Memos instance.** TypeScript was chosen. The four core tools work over both local stdio and local HTTP. Semantic search / SQLite indexing / time-retrieval are not yet built (see Roadmap).
...
Step 1 done: `memos_list` / `memos_get` / `memos_search` / `memos_create`, local stdio + local HTTP, auth abstraction, normalization, permission gate.
